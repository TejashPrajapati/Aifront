...
//         // console.log("Received blogs:", response.data.map(blog => ({ id: blog._id, title: blog.title })));
...
//                 // console.log("Rendering blog card:", { id: blog._id, title: blog.title });
//                 return (
//                   <Link
//                     key={blog._id}
//                     to={`/blog/${blog._id}`}
//                     onClick={() => console.log("Clicked blog:", { id: blog._id, title: blog.title })}
...
//                         {new Date(blog.createdAt).toLocaleDateString()} | {blog.priceType === 'free' ? 'Free' : 'Paid'}
//                       </p>
//                     </div>
//                   </Link>
//                 );
          ...
          <ul className="space-y-2">
            {categories.map((category) => (
              <li
                key={category}
                onClick={() => handleCategoryChange(category)}
                className={`cursor-pointer p-3 rounded-lg transition-colors duration-300 ${selectedCategory === category ? 'bg-blue-500 text-white' : 'bg-white hover:bg-gray-200'
                  }`}
              >
                {category}
              </li>
                      ...
                      <div className="flex items-center mb-4">
                        <div className="flex items-center">
                          {[...Array(5)].map((_, index) => (
                            <svg
                              key={index}
                              className={`w-4 h-4 ${index < blog.rating ? 'text-yellow-400' : 'text-gray-300'
                                }`}
                              fill="currentColor"
                              viewBox="0 0 20 20"
                            >
